...
export default function ChatWidget() {
    ...
    const sendMessage = async () => {
        ...
        try {
            await axios.post('/api/chats', { message: msg });
            loadMessages();
        } catch (error) {
    ...
    // Inline styles ported from Blade to style object or keeping classes if possible.
    // The original used IDs and strict CSS. We will use a mix of style objects and className.
    // Ideally we should move this to a CSS file, but for portability in this task I'll use objects where classes are missing.

    const chatWindowStyle = {
        position: 'fixed', // Changed from absolute to fixed to match widget container context or use fixed for window
        bottom: '80px',
    ...
    // Check dark mode for background override (simple check via class on body, 
    // but React won't re-render on class change unless we listen to it. 
    // For now we assume light mode default for styles or rely on CSS classes if we can).
    // Better to use className 'chat-window' and let CSS handle it.
        ...
        <div id="chat-widget" style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 9999 }}>
            <button
                ...
                style={{
                    ...
                    backgroundColor: '#6f42c1' // Default purple usually
                }}
            >
